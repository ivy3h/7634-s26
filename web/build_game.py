...
function renderDmLog() {
  ...
  const entries = dmVerbose
    ? [...state.dmLog].reverse()
    : [...state.dmLog].reverse().filter(e => e.kind === "constituent" || e.kind === "exceptional" || e.kind === "hard_block");
  ...
  entries.forEach(e => {
    ...
    if (e.kind === "constituent") {
      const desc = esc(e.event_desc || e.summary || "");
      const reveals = (e.reveals_descs || []).map(d => esc(d)).join("; ");
      div.innerHTML = `<span class="tag">[${tLabel} ✓]</span> ${desc}`
        + (reveals ? `<br><span style="color:var(--muted);font-size:0.82em">Reveals: ${reveals}</span>` : "");
      div.title = e.detail || "";
    } else if (e.kind === "exceptional") {
      const rationale = esc((e.rationale || "").slice(0, 100));
      const removed = (e.removed_descs || []).map(d => esc(d)).join("; ");
      const added   = (e.added_descs   || []).map(d => esc(d)).join("; ");
      const goal    = e.goal_verdict ? ` — goal: ${esc(e.goal_verdict)}` : "";
      div.innerHTML = `<span class="tag">[${tLabel} ⚠ accommodated]</span> DM accepted action + updated plan${goal}`
        + (rationale ? `<br><span style="color:var(--muted);font-size:0.82em">Why: ${rationale}</span>` : "")
        + (removed   ? `<br><span style="color:var(--muted);font-size:0.82em">Removed: ${removed}</span>` : "")
        + (added     ? `<br><span style="color:var(--muted);font-size:0.82em">Added: ${added}</span>`   : "");
      div.title = e.detail || "";
    } else if (e.kind === "hard_block") {
      div.innerHTML = `<span class="tag">[${tLabel} ✗ refused]</span> DM blocked action — world unchanged`;
      div.title = e.detail || "";
    } else {
      // consistent / move / other — concise single line
      div.innerHTML = `<span class="tag">[${tLabel} ${e.kind === "move" ? "→" : "·"}]</span> ${esc(e.summary || e.kind)}`;
      div.title = e.detail || "";
    }
  ...
  try {
    ...
    if (data.dm_entry) {
      state.dmLog.push({
        ...
        reveals_descs: data.dm_entry.reveals_descs|| [],
        removed_descs: data.dm_entry.removed_descs|| [],
        added_descs:   data.dm_entry.added_descs  || [],
        rationale:     data.dm_entry.rationale    || "",
